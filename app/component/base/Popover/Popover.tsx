...
const Popover = forwardRef<PopoverInstance, PopoverProps>((props, ref) => {
  ...
  // const { styles, theme: defaultTheme } = useThemeFactory(createStyle, theme);
  const touchable = useRef(null);
  const [visible, setVisible] = React.useState(false);

  const actionActiveBackgroundColor =
    theme === "light" ? THEME_VARIABLE.active_color : "rgba(0, 0, 0, 0.2)";
  ...
  const arrowSize = useMemo(
    () => ({
      width: defaultTheme.popover_arrow_size * 2,
      height: defaultTheme.popover_arrow_size,
    }),
    [defaultTheme.popover_arrow_size]
  );
          ...
          {icon && (
            <View style={styles.iconWrapper}>
              {React.cloneElement(icon as React.ReactElement, {
                color: textColor,
                size: defaultTheme.popover_action_icon_size,
              })}
            </View>
          )}
          <Text

...
export default function App() {
  ...
  const [apiKey, onChangeApiKey] = React.useState("f9e1ab46abdff9fa95bb");
  const [cluster, onChangeCluster] = React.useState("ap3");
  const [channelName, onChangeChannelName] = React.useState("presence-test");
  const [eventName, onChangeEventName] = React.useState("some");
  ...
  const onAuthorizer = async (channelName: string, socketId: string) => {
    return JSON.stringify({ channel_data: "dddd" })  as PusherAuthorizerResult;
    log(
  ...
  const trigger = async () => {
    try {
      ...
      console.log(pusher.channels, "pusher.channels");
      console.log(channelName, eventName, eventData);

      await pusher.trigger({ channelName, eventName, data: eventData });
      console.log("finish");
    } catch (e) {
      log("ERROR: " + e);
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      {/* <Image style={styles.image} source={require('./pusher.png')} />npx */}
      <View>
        <Text>
